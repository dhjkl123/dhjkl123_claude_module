...
3. Treat reviewed-spec confirmation as mode-sensitive:
   ...
   - in `spec-only` and `spec+draft-mock`, reviewed-spec confirmation is optional and must not be treated as a mandatory gate unless the user explicitly asks for that reviewed-spec path
...
5. When `artifacts/reviewed-spec.yaml` is created:
   ...
   - set `review.review_status: current`
   - if this replaces a previously current reviewed spec for another revision, record that prior revision as `superseded` or `invalidated` in `artifacts/review-log.yaml`
   - preserve links back to:
     - original state entries
     - source candidate and evidence references when retained by the template
     - reviewed trigger entries and their source refs when retained by the template
     - reviewer decisions
     - unresolved blockers
6. Do not treat artifact existence alone as reviewed validity.
   ...
   - `artifacts/reviewed-spec.yaml` is the canonical current reviewed spec slot for the session
   - if a later spec revision changes the current spec, replace the canonical file for the new revision and record the prior reviewed revision as `superseded` or `invalidated` in `artifacts/review-log.yaml`
...
- if the path is draft, any unresolved material evidence gap that affects the mock is either user-approved for the same fallback scope or treated as blocking
- if the path is reviewed, there are no unresolved material evidence gaps
