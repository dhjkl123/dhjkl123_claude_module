...
5. Create required subpaths under the same session root before writing YAML artifacts:
   ...
   - `plans/` only when the selected mode is `spec+implementation-plan`
...
9. Use the canonical review log at `artifacts/review-log.yaml`.
   ...
   - If the file does not exist for the current session, initialize it at that canonical path.
...
12. Record in `artifacts/session-scope.yaml`:
   - `file_key`
   - `scope_type`
   - the type-specific scope identifiers such as `section_node_id`, `frame_node_ids`, `flow_start_frame_node_id`, or the explicit related-frame-set basis
   - `scope_state: temporary`
   - `output_mode`
   - `artifact_root`
   - `session_id`
   - `spec_revision_id`
...
16. Treat `scope confirmation` as the step completion gate:
   ...
   - after explicit confirmation, update the scope to confirmed and continue
...
17. If any `assumption` or `open_scope_question` is already a known material evidence gap, do not allow it to silently flow into a user-facing interpretation, implementation plan, fallback decision, or mock path without the required approval.
...
- the selected mode is one of the four allowed modes
